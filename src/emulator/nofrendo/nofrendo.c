...
static int internal_insert(const char *filename, system_t type)
{
   ...
   case system_nes:
      ...
      /* Use NES_SCREEN_HEIGHT (240) not NES_VISIBLE_HEIGHT (224):
      ** ppu_scanline renders scanlines 0..239, so the bitmap must have
      ** at least 240 line[] entries or we get an out-of-bounds read. */
...
int main_loop(const char *filename, system_t type)
{
   ...
   ** last run. Without this reset the while-loop below exits immediately. */
   ...
   /* event_init() is normally called by nofrendo_main(), but we call
   ** main_loop() directly — so initialise the input system here. */
   event_init();
