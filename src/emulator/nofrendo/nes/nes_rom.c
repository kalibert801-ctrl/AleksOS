...
void rom_free(rominfo_t **rominfo)
{
   ...
   if ((*rominfo)->sram)
      free((*rominfo)->sram);
   /* rom and vrom point into the osd_getromdata() PSRAM buffer — NOT
   ** individually malloc'd.  Freeing them would corrupt the heap.
   ** The buffer is released by osd_rom_free() after main_loop returns. */
