...
**AleksOS v16** — портативная игровая консоль на базе **ESP32-2432S028** («Cheap Yellow Display»).  
NES-эмулятор **Nofrendo** с полностью переработанным интерфейсом, веб-менеджером, виброотдачей, BLE геймпадом и поддержкой WiFi.
...
- Скриншоты во время игры — первый скриншот автоматически становится обложкой
- Галерея скриншотов доступна через веб-интерфейс
...
### 2-player BLE геймпад (Android)

Второй игрок управляет через **Android-приложение** по Bluetooth Low Energy — без сопряжения через настройки телефона.

- Консоль автоматически рекламируется как BLE-устройство `AleksOS` при входе в 2-player режим
- Приложение сканирует и подключается напрямую; уже сопряжённые устройства тоже определяются
- Кнопки D-pad, A, B, SELECT, START передаются в реальном времени как один байт по Nordic UART Service (NUS)
- Сборка приложения: папка `Android/`, Flutter + flutter_blue_plus
...
- **Bluetooth:** NimBLE-Arduino (Apache NimBLE, BLE NUS-геймпад)
- **Конфиг:** ArduinoJSON на SD карте
- **NTP:** ip-api.com (часовой пояс по IP) + pool.ntp.org
- **Android-приложение:** Flutter + flutter_blue_plus
...
## Android-приложение (2-player BLE Controller)

Приложение-геймпад для второго игрока — папка `Android/`.

**Требования:** Flutter SDK, Android Studio или ADB
...
**Как использовать:**
1. Включить ESP32, войти в 2-player режим — консоль начнёт рекламироваться по BLE
2. Открыть приложение на Android → нажать **Подключить**
3. Выбрать `AleksOS` из списка — подключение без сопряжения
4. Управлять D-pad и кнопками A/B/SELECT/START

> **Протокол:** Nordic UART Service (NUS, UUID `6E400001...`), один байт со статусом кнопок.  
> Биты: `A=0x01 B=0x02 SELECT=0x04 START=0x08 ↑=0x10 ↓=0x20 ←=0x40 →=0x80`
